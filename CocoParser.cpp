...
void Parser::module_declaration() {
		...
		if (la->kind == _T_Lpar) {
			...
			addTerminal(); 
			if (StartOf(2)) {
				if (la->kind == _T_Dot || la->kind == _T_Lbrace || la->kind == _T_Ident) {
					list_of_ports();
				} else {
...
void Parser::gate_instantiation() {
		...
		case _T_bufif0: case _T_bufif1: case _T_notif0: case _T_notif1: {
			enable_gatetype();
			if (peek(1) == _T_Lpar && ( peek(2) == _T_pull0 || peek(2) == _T_weak0 || peek(2) == _T_pull1 || peek(2) == _T_supply1 || peek(2) == _T_weak1 || peek(2) == _T_highz0 || peek(2) == _T_highz1 || peek(2) == _T_strong0 || peek(2) == _T_strong1 || peek(2) == _T_supply0 ) ) {
				drive_strength();
			}
			if (la->kind == _T_Hash) {
				delay3();
		...
		case _T_pulldown: {
			Get();
			addTerminal(); 
			if (peek(1) == _T_Lpar && ( peek(2) == _T_pull0 || peek(2) == _T_weak0 || peek(2) == _T_pull1 || peek(2) == _T_supply1 || peek(2) == _T_weak1 || peek(2) == _T_strong0 || peek(2) == _T_strong1 || peek(2) == _T_supply0 ) ) {
				pulldown_strength();
			}
			pull_gate_instance();
			while (la->kind == _T_Comma) {
...
void Parser::primary() {
		if (la->kind == _T_SysName || la->kind == _T_Ident) {
			if (la->kind == _T_Ident) {
				hierarchical_identifier_range();
			} else {
...
void Parser::based_number() {
		Expect(_T_BasedInt,__FUNCTION__);
		addTerminal(); 
}

void Parser::base_format() {
		Expect(_T_BaseFormat,__FUNCTION__);
		addTerminal(); 
}

void Parser::base_value() {
		Expect(_T_BaseValue,__FUNCTION__);
		addTerminal(); 
}

void Parser::sizedbased_number() {
		Expect(_T_SizedBased,__FUNCTION__);
		addTerminal(); 
}
